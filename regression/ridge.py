...
class Ridge(Model):
    def __init__(self, learning_rate=0.01, alpha=0.11, epochs=1000, method="default", tol=0.001):
        self.fitted = False
    ...
    def predict(self, X:np.array):
        if (X.shape[1] != self.features):
            raise Exception("Ridge: Shape should be", self.features, "and not", X.shape[1])
        else:
    ...
    def __compute_stochastic_gradient(self):
        for m in range(self.X.shape[0]):
            ...
            self.gradients = 2 * self.X[rand_index:rand_index+1].T.dot(self.X[rand_index:rand_index+1].dot(self.weights) - self.y[rand_index:rand_index+1]) + 2 * self.alpha * self.weights

    def __compute_gradient(self):
        self.gradients = 2 * self.X.T.dot(self.X.dot(self.weights) - self.y) + 2 * self.alpha * self.weights

    def __train(self, epoch):
        if not self.fitted:
            raise Exception("Ridge: Model not fitted with data")
        elif self.y.shape[0] != self.X.shape[0] or self.y.shape[1] != 1:
            raise Exception("Ridge: Invalid shapes Xshape", self.X.shape, "while yshape", self.y.shape)
        else:
    ...
    def __train_analytic(self):
        if not self.fitted:
            ...
            raise Exception("Ridge: Invalid shapes X", self.X.shape, "while y", self.y.shape)
        else:
            self.weights = np.linalg.inv(self.X.T.dot(self.X) + self.alpha * self.id).dot(self.X.T.dot(self.y))